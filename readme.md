...
## Overview
The goal of this project is to build a map of surrounding area and autonomosly controled a robot to the pointed destination in the map. The robot is a Turtlebot model and base on ROS (Robot Operating System). This project focuses on using Sereo camera to reduce the cost. Robot can build a map, navigate and go to destinated position, which can be futher developed to go around a house doing diffrent tasks or exploring unknown enviroment. 
